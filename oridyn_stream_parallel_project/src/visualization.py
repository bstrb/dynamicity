...
def plot_detector_frame(
    ...
    if frame_table.empty:
        vmin = 0.0 if score_vmin is None else float(score_vmin)
        vmax = 1.0 if score_vmax is None else float(score_vmax)
    else:
        vmin = float(frame_table[score_column].min()) if score_vmin is None else float(score_vmin)
        vmax = float(frame_table[score_column].max()) if score_vmax is None else float(score_vmax)
    if vmax <= vmin:
        vmax = vmin + 1.0
