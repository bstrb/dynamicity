...
def _excitation_error(g_vectors: FloatArray, wavelength_angstrom: float) -> FloatArray:
    inv_lambda = 1.0 / float(wavelength_angstrom)
    gx = g_vectors[:, 0]
    gy = g_vectors[:, 1]
    gz = g_vectors[:, 2]
    return np.sqrt(gx * gx + gy * gy + (inv_lambda + gz) ** 2) - inv_lambda
...
def canonical_observations_from_xds(
    ...
    integrate_path: str | Path | None = None,
    ...
    if integrate_path is not None:
        ...
        integrate = rprofall_to_integrate_data(parse_rprofall(rprofall_path))
...
def canonical_observations_from_pets(
    pets_project_path: str | Path,
    pets_rprofall_path: str | Path | None = None,
    dataset_id: str | None = None,
) -> CanonicalObservationData:
    """Build canonical observations from PETS project + rprofall."""

    pets = parse_pets_project(pets_project_path, rprofall_path=pets_rprofall_path)
    gxparm, integrate, reciprocal_by_frame = pets_project_to_analysis_inputs(pets)
    dataset = dataset_id or Path(pets_project_path).stem
    if dataset.endswith(".backup"):
        dataset = dataset[: -len(".backup")]
    return _canonical_from_core_inputs(
        gxparm=gxparm,
        integrate=integrate,
        reciprocal_by_frame=reciprocal_by_frame,
...
def _ori_thick_metrics_for_frame(
    ...
    for p_idx in range(n_pert):
        ...
        g = hkls @ ub_pert.T
        sg = _excitation_error(g, wavelength_angstrom)
    ...
    sg_center = _excitation_error(g_center, wavelength_angstrom)
...
def run_dynamical_uncertainty_from_pets(
    pets_project_path: str | Path,
    pets_rprofall_path: str | Path | None = None,
    dataset_id: str | None = None,
    config: DynamicalUncertaintyConfig | None = None,
) -> DynamicalUncertaintyResult:
    """Run uncertainty screening from PETS project input."""

    canonical = canonical_observations_from_pets(
        pets_project_path=pets_project_path,
        pets_rprofall_path=pets_rprofall_path,
        dataset_id=dataset_id,
    )
    return run_dynamical_uncertainty_pipeline(canonical, config=config)
