"""Export one selected frame as a 3D Gaussian diffraction pattern HTML."""
...
from src.html_visualization import export_single_frame_gaussian_html
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--frame-number", type=int, required=True, help="One-based frame number (for example 45)")
    parser.add_argument("--score-column", default="S_comb", help="Score column mapped to gaussian sigma and color")
    ...
    parser.add_argument("--surface-grid-size", type=int, default=170, help="Grid resolution for 3D gaussian surface")
    parser.add_argument("--min-sigma-px", type=float, default=8.0, help="Minimum gaussian width in pixels")
    parser.add_argument("--max-sigma-px", type=float, default=42.0, help="Maximum gaussian width in pixels")
    parser.add_argument(
        "--overlap-mode",
        choices=["max", "sum"],
        default="max",
        help="How overlapping gaussians are combined in the surface",
    )
    parser.add_argument("--surface-opacity", type=float, default=0.78, help="Opacity of the 3D gaussian surface")
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.03,
        help="Hide tails below this fraction of max height",
    )

    parser.add_argument(
        "--output-html",
        default="analysis_output/frame_gaussian_3d.html",
...
def main() -> None:
    ...
    output_path = export_single_frame_gaussian_html(
        result=result,
    ...
    print(f"Wrote single-frame 3D gaussian report to: {output_path}")
