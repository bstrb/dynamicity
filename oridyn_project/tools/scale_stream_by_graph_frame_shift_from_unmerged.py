#!/usr/bin/env python3
"""Scale CrystFEL stream reflections using graph+frame risk derived from unmerged data.

This script rewrites an input stream in place only conceptually: it writes a new stream
where selected high-risk observations are down-scaled based on a per-HKL comparison of
low-risk and high-risk unmerged intensities joined to OriDyn scores.
...
the supplied stream using the per-observation scale factors inferred from the supplied
unmerged HKL and score table.
"""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        action="store_true",
        help="Do not target locally weak HKLs whose low-risk reference median I_ref is <= 0",
    )
...
def compute_hkl_diagnostics(
    ...
    min_scale: float,
    max_scale: float,
    skip_weak_nonpositive_iref: bool,
    progress_every: int,
    ...
    selected_observation_scales: dict[str, float] = {}
    ...
    if valid.empty:
        ...
        return out, {"eligible_signed_hkls": 0, "target_weak_signed_hkls": 0, "scale_factors": []}, selected_observation_scales
    ...
    skipped_nonpositive_iref = weak_positive_shift & (i_ref_values <= 0.0)
    if skip_weak_nonpositive_iref:
        work["eligible"] = weak_positive_shift & (i_ref_values > 0.0)
    else:
        work["eligible"] = weak_positive_shift
    ...
    for eligible_idx, (idx, row) in enumerate(eligible_rows.iterrows(), start=1):
        ...
        for key in selected["key"].astype(str):
            selected_observation_scales[key] = scale

    diagnostics = {
        "eligible_signed_hkls": int(work["eligible"].sum()),
        "weak_positive_shift_hkls_before_iref_filter": int(weak_positive_shift.sum()),
        "weak_nonpositive_iref_hkls_skipped": int(skipped_nonpositive_iref.sum()) if skip_weak_nonpositive_iref else 0,
        "target_weak_signed_hkls": int((work["eligible"] & (work["graph_frame_scale"] < 1.0)).sum()),
        "selected_top_risk_observation_keys": int(len(selected_observation_scales)),
        "scale_factors": scale_factors,
    }
    work = work.drop(columns=["eligible", "reciprocal_length"])
    return work, diagnostics, selected_observation_scales
...
def main() -> int:
    ...
    per_hkl, diag, selected_observation_scales = compute_hkl_diagnostics(
        ...
        skip_weak_nonpositive_iref=bool(args.skip_weak_nonpositive_iref),
    ...
    for ref in stream_refs:
        scale = float(selected_observation_scales.get(ref.key, 1.0))
        if scale != 1.0:
    ...
    scale_summary = {
        ...
        **diag,
        ...
        "version": "graph_frame_shift_from_unmerged_v1",
    }
