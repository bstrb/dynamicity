...
SCORES=$BASE/oridyn_v2_scoring_filtering_20_0p5_20260623/v3_excited_lowcoupling_scores/geometry_coupling_v3_excited_lowcoupling_scores.csv
OUTDIR=$BASE/oridyn_v3_excited_lowcoupling_filtering_20_0p5_20260704
RISK_COLUMN=badness_v3_excited_lowcoupling_shell_norm
...
usage() {
  cat <<'EOF'
Usage: tools/run_v3_excited_lowcoupling_filter_sweep.sh [--dry-run] [--force] [--drop-low-count-hkls]

Filter the MFM300-VIII stream by the v3 excited-lowcoupling badness score at
keep80, keep60, keep40, and keep20. By default, signed HKLs with fewer than the
filter tool's --min-obs-per-hkl are kept unchanged to avoid dropping sparse HKLs
by floor(n * keep_fraction) alone.
EOF
}
...
prepare_output_paths() {
  ...
  local outstream=$OUTDIR/MFM300_VIII_v3_excited_lowcoupling_${tag}.stream
  [[ "$FORCE" == true || ! -e "$outstream" ]] || die "output stream exists; re-run with --force to replace: $outstream"
  [[ "$FORCE" == true || ! -e "$workroot" ]] || die "tool output directory exists; re-run with --force to replace: $workroot"
}
...
run_keep_fraction() {
  ...
  local workroot=$OUTDIR/tool_outputs/$tag
  local outstream=$OUTDIR/MFM300_VIII_v3_excited_lowcoupling_${tag}.stream
  local logfile=$OUTDIR/logs/filter_v3_excited_lowcoupling_${tag}.log
  ...
  if [[ "$KEEP_LOW_COUNT_HKLS" == true ]]; then
    cmd+=(--keep-low-count-hkls)
  fi

  log "Starting v3 excited-lowcoupling filtering: $tag ($keep_fraction)"
