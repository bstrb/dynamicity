#!/usr/bin/env python3
"""Filter a CrystFEL stream by per-HKL geometry-coupling trust risk.

For each signed HKL, this diagnostic keeps the lowest-risk observations and
removes the highest-risk tail for one or more keep fractions. Matching is by
exact source_filename + event + signed h,k,l keys. HKLs are not canonicalized.

This is a cSerialED geometry-risk filtering experiment, not an intensity
correction. Reflection intensities, sigmas, HKL indices, and non-reflection
stream text are preserved for observations that are kept.
"""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        nargs="+",
        type=float,
        default=DEFAULT_KEEP_FRACTIONS,
        help="Per-signed-HKL fractions to keep, e.g. 0.90 0.80 0.70 0.60 0.50",
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help="Optional smoke-test limit on stream crystal blocks/events processed",
    )
...
def variant_name(fraction: float) -> str:
    return f"geometry_trust_keep{percent_label(fraction)}"
...
def collect_smoke_keys(stream_path: Path, max_events: int, progress_every: int) -> tuple[set[tuple[str, str, int, int, int]], dict[str, int]]:
    """Collect exact observation keys from the first max_events crystal blocks."""
...
def normalize_score_chunk(chunk: pd.DataFrame) -> pd.DataFrame:
    out = chunk.copy()
...
def filter_chunk_to_keys(chunk: pd.DataFrame, key_filter: set[tuple[str, str, int, int, int]]) -> pd.DataFrame:
    ...
    mask = [key in key_filter for key in keys]
...
def load_score_table(
    ...
    require_columns(header, [*KEY_COLUMNS, *SCORE_COLUMNS], "scores CSV")
    usecols = [*KEY_COLUMNS, *SCORE_COLUMNS]
    stats = {
        "score_rows_read": 0,
    ...
    if key_filter is None:
        ...
        table = normalize_score_chunk(raw)
        stats["score_rows_after_hkl_cleanup"] = int(len(table))
    else:
        chunks = []
        for idx, chunk in enumerate(pd.read_csv(scores_path, usecols=usecols, chunksize=SCORE_CHUNKSIZE), start=1):
            ...
            normalized = normalize_score_chunk(chunk)
...
def robust_normalize(values: pd.Series) -> tuple[pd.Series, dict[str, float | int | str]]:
    ...
    if not np.isfinite(p01) or not np.isfinite(p99) or p99 <= p01:
        ...
        normalized.loc[values.isna()] = np.nan
        method = "degenerate_p01_p99_to_zero"
    else:
        normalized = ((values - p01) / (p99 - p01)).clip(lower=0.0, upper=1.0)
...
def compute_trust_scores(table: pd.DataFrame) -> tuple[pd.DataFrame, dict[str, Any]]:
    work = table.copy()
    for column in SCORE_COLUMNS:
        work[column] = pd.to_numeric(work[column], errors="coerce").replace([np.inf, -np.inf], np.nan)
    work["manybeam_exposure_proxy"] = (
        work["graph_crowding_norm"]
        * (1.0 + 0.5 * work["same_laue_zone_crowding_norm"])
        * (1.0 + 0.5 * work["systematic_row_risk_norm"])
        * (1.0 + 0.5 * work["frame_axis_risk_norm"])
    )
    work["trust_risk_raw"] = np.log1p(work["manybeam_exposure_proxy"])
    work["trust_risk_raw"] = pd.to_numeric(work["trust_risk_raw"], errors="coerce").replace([np.inf, -np.inf], np.nan)
    work["trust_risk_norm"], normalization = robust_normalize(work["trust_risk_raw"])
    finite = work["trust_risk_norm"].map(np.isfinite)
    dropped = int((~finite).sum())
    if dropped:
        log(f"Dropping {dropped:,} score rows with non-finite trust_risk_norm")
    work = work.loc[finite].copy()
    duplicates = work.duplicated(KEY_COLUMNS, keep=False)
    duplicate_rows = int(duplicates.sum())
    duplicate_keys = int(work.loc[duplicates, KEY_COLUMNS].drop_duplicates().shape[0]) if duplicate_rows else 0
    if duplicate_rows:
        log("Warning: duplicate exact observation keys in scores; keeping the first row per key")
    ...
    metadata = {
        "formula": (
            ...
            "(1 + 0.5*frame_axis_risk_norm); trust_risk_raw = log(1 + manybeam_exposure_proxy)"
        ),
        "normalization": normalization,
        "nonfinite_trust_rows_dropped": dropped,
        "duplicate_score_rows": duplicate_rows,
        "duplicate_score_keys": duplicate_keys,
        "unique_scored_observation_keys": int(len(work)),
    }
...
def build_filter_masks(
    ...
    work = scored.sort_values([*HKL_COLUMNS, "trust_risk_norm", "source_filename", "event"], kind="mergesort").reset_index(drop=True)
    grouped = work.groupby(HKL_COLUMNS, sort=False)
    work["_n_total"] = grouped["trust_risk_norm"].transform("size").astype("int64")
    work["_risk_rank"] = grouped.cumcount().astype("int64")
    work["_low_count_hkl"] = work["_n_total"] < int(min_obs_per_hkl)

    n_low_count_hkls = int(work.loc[work["_low_count_hkl"], HKL_COLUMNS].drop_duplicates().shape[0])
    n_scored_hkls = int(work[HKL_COLUMNS].drop_duplicates().shape[0])
    ...
    log("Building exact-key removal bitmask lookup")
    ...
    variant_selection = pd.DataFrame.from_records(variant_rows)
    stats = {
        "scored_hkls": n_scored_hkls,
        "low_count_hkls": n_low_count_hkls,
        "low_count_hkls_kept_unchanged": n_low_count_hkls if keep_low_count_hkls else 0,
        "key_to_mask_entries": int(len(key_to_mask)),
    }
    return key_to_mask, variant_selection, stats


def output_paths(output_root: Path, keep_fractions: list[float]) -> dict[str, dict[str, Path]]:
    paths: dict[str, dict[str, Path]] = {}
    for fraction in keep_fractions:
        name = variant_name(fraction)
        paths[name] = {
...
def write_stream_variants(
                    ...
                    if hkl is not None:
                        ...
                        if stream_observations_seen % int(progress_every) == 0:
                            ...
                            log(
                                ...
                                f"matched={matched_observations:,}, "
                                f"unmatched={unmatched_observations:,}, "
                                f"{removed_text}"
                            )
                        continue
    ...
    for variant in variants:
        ...
        row["variant"] = variant
        row["keep_fraction"] = float(keep_fractions[variants.index(variant)])
    ...
    return pd.DataFrame.from_records(summary_rows), stream_stats, removed_by_hkl, matched_hkl_counts
...
def write_run_metadata(
    ...
    metadata = {
        ...
        "stream_stats": stream_stats,
        "warnings": [
            "This is cSerialED geometry-risk filtering, not an intensity correction.",
            ...
            "This tool only prepares filtered streams; merging/refinement is intentionally left to the user.",
        ],
    }
...
def main() -> int:
    ...
    log("Loading score table and computing geometry trust-risk score")
    score_table, score_load_stats = load_score_table(args.scores, smoke_key_filter)
    score_load_stats.update(smoke_stats)
    scored, score_metadata = compute_trust_scores(score_table)
    log(
        "Score table ready: "
        f"unique_scored_keys={score_metadata['unique_scored_observation_keys']:,}, "
        f"trust_p01={score_metadata['normalization']['p01']:.6g}, "
        f"trust_p99={score_metadata['normalization']['p99']:.6g}"
    )
    ...
    log("Writing filtered stream variants in one stream pass")
    ...
    sweep_summary = sweep_summary.merge(variant_selection, on=["variant", "keep_fraction"], how="left")
    sweep_summary.to_csv(args.output_root / "filter_sweep_summary.csv", index=False)

    write_variant_jsons(paths, sweep_summary, stream_stats, selection_stats, score_metadata, args)
    write_run_metadata(args.output_root, args, score_load_stats, score_metadata, selection_stats, stream_stats)

    log("Filtering sweep complete")
    for variant_paths in paths.values():
