...
Adds:
- After renaming outputs, append a single line to runs/progress_log.csv
  with (run_n, total_chunks, indexed_by_header, index_rate, wrmsd_summary).
...
import argparse, os, sys, re, subprocess, shutil, csv
...
def evaluate_stream_with_step3(run_dir: str, step3_path: str) -> int:
    ...
    # --- NEW: append a progress line from summary_NNN.txt into runs/progress_log.csv
    summary_path = os.path.join(run_dir, f"summary_{n:03d}.txt")
    prog_csv = os.path.join(os.path.dirname(run_dir), "progress_log.csv")
    try:
        total = idx_hdr = idx_rate = wrmsd_line = ""
        if os.path.isfile(summary_path):
            with open(summary_path, "r", encoding="utf-8") as sf:
                for ln in sf:
                    if ln.startswith("Total chunks:"):
                        total = ln.split(":",1)[1].strip()
                    elif ln.startswith("Indexed (by header):"):
                        idx_hdr = ln.split(":",1)[1].strip()
                    elif ln.startswith("Index rate:"):
                        idx_rate = ln.split(":",1)[1].strip()
                    elif ln.startswith("WRMSD:"):
                        wrmsd_line = ln.split(":",1)[1].strip()
        write_header = not os.path.exists(prog_csv)
        with open(prog_csv, "a", newline="", encoding="utf-8") as f:
            w = csv.writer(f)
            if write_header:
                w.writerow(["run_n","total_chunks","indexed_by_header","index_rate","wrmsd_summary"])
            w.writerow([f"{n:03d}", total, idx_hdr, idx_rate, wrmsd_line])
        print(f"[eval] appended progress to {prog_csv}")
    except Exception as e:
        print(f"[eval] WARNING could not update progress_log.csv: {e}", file=sys.stderr)
