...
# -*- coding: utf-8 -*-
"""
step4_prepare_next_run_overlay_elink.py

Iterative Step 4 using per-source overlays with ExternalLink (no VDS).

Adds:
- Early-break stream writer: best chunk per image so far across runs
- Per-image logs (CSV): all tried shifts & outcomes appended each iteration
- Local search: single-probe-per-run 2D Nelder–Mead (simplex) around current best

Process each time it's run:
1) Auto-detect latest run_NNN under <run_root>/runs/
2) Read lst_NNN.lst (pairs of <h5> // <frame>), chunk_metrics_NNN.csv
3) Update persistent per-image state from latest results
4) Write/append per-image logs from latest results
5) Write/update early-break stream that contains best-per-image chunks so far
6) Decide ONE candidate shift per image for the next run (ring or NM-local)
7) Create overlays for each source H5 and write candidates for the frames used
8) Emit lst_(NNN+1).lst and sh_(NNN+1).sh
...
def resolve_real_source(h5_path: str) -> str:
    ...
    try:
        with h5py.File(h5_path, "r") as f:
            # get the link object, not the target dataset
            link = f.get("/entry/data/images", getlink=True)
            if isinstance(link, h5py.ExternalLink):
                # link.filename is whatever was stored when overlay was created
...
# Nelder–Mead parameters (fixed for simplicity)
    ...
    if os.path.isfile(path):
        with open(path, "r", encoding="utf-8") as f:
            ...
            for k, v in raw.items():
                # compatibility with previous state versions
...
def _ensure_nm_initialized(S: ImgState) -> None:
    if S.nm_initialized:
        return
    # Initialize simplex around current best (or last) with step = S.nm_step
    cx = S.best_dx if S.best_dx is not None else S.last_dx
    cy = S.best_dy if S.best_dy is not None else S.last_dy
    S.nm_vertices = [
        NMVertex(cx, cy, f=S.best_wrmsd, done=(S.best_wrmsd is not None)),   # v0: best so far
        NMVertex(cx + S.nm_step, cy, f=None, done=False),                    # v1: to be evaluated
        NMVertex(cx, cy + S.nm_step, f=None, done=False)                     # v2: to be evaluated
    ]
    S.nm_initialized = True

def pick_local_nm_probe(S: ImgState, seed: int) -> Tuple[float,float,str]:
    """
    One-probe-per-run Nelder–Mead:
    - If simplex has unevaluated vertex -> propose it
    - Else perform a reflection step and propose reflected point
    """
    _ensure_nm_initialized(S)
    # Any unevaluated vertex?
    for v in S.nm_vertices:
        if not v.done:
            S.nm_last_proposal = (v.dx, v.dy)
            return v.dx, v.dy, "nm_eval_vertex"
    # All evaluated -> do a single NM step (reflection)
    # Order vertices by f (ascending)
...
def update_state_from_run(S: ImgState, run_n: int, chunk_id: int, was_indexed: bool, wrmsd: Optional[float],
    ...
    if was_indexed and wrmsd is not None and math.isfinite(wrmsd):
        ...
        # If we have an NM simplex and last proposal matches a vertex, record f
...
def parse_stream_chunks(stream_path: str) -> List[Tuple[int,int]]:
    """
    Return list of (start_line_index, end_line_index_exclusive) for each chunk in the stream.
    """
    bounds = []
    start = None
...
def write_early_break_stream(run_root: str, latest_run_n: int, state: Dict[int, ImgState]) -> str:
    """
    Build a stream that contains the current best chunk per image from the run that produced it.
    """
    runs_dir = os.path.join(run_root, "runs")
    out_path = os.path.join(runs_dir, f"early_break_{latest_run_n:03d}.stream")
    pieces: List[str] = []
    # Group best chunks by their originating run to avoid reopening files too often
    ...
    # Extract chunk text from each run's stream
...
def main(argv: List[str]) -> int:
    ...
    def _src_from_image_col(img: str) -> str:
        # "image" column is like "<path> //<frame>" from the stream.
        # Return just the path; we'll normalize to real source via resolve_real_source.
        if "//" in img:
            h5, _ = img.split("//", 1)
            return h5.strip()
        return img.strip()

    # Map (real_source, event) -> row
    ...
    for gi in range(N):
        src, fr = pairs[gi]                          # pairs parsed from lst_NNN.lst
        real = resolve_real_source(src)
        row = rows_by_key.get((real, fr), {})
        ...
        cid = int(row.get("chunk_id", gi) or gi)  # fallback to gi if missing
        ...
        with open(logp, "a", encoding="utf-8", newline="") as f:
            ...
            w.writerow([last_n, gi, state[gi].phase, f"{tried_dx:.6f}", f"{tried_dy:.6f}", int(was_indexed),
    ...
    if not any_candidates:
        ...
        with open(final_csv, "w", encoding="utf-8", newline="") as f:
            w = csv.writer(f); w.writerow(["global_index","best_wrmsd","best_dx_mm","best_dy_mm","phase","done","give_up","best_run","best_chunk_id"])
            for gi in range(N):
                S = state[gi]
                w.writerow([gi, S.best_wrmsd if S.best_wrmsd is not None else "",
                               S.best_dx if S.best_dx is not None else "",
                               S.best_dy if S.best_dy is not None else "",
                               S.phase, int(S.done), int(S.give_up), S.best_run if S.best_run is not None else "", S.best_chunk_id if S.best_chunk_id is not None else ""])
    ...
    # write lst for next run (pointing to overlays)
