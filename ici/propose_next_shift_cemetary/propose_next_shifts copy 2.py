...
def constrained_bo_next(XY: np.ndarray,
    ...
    # Guard-rails: keep only plausible or uncertain points, and keep arrays in sync
    keep = (mu_m <= guard_mu) | (var_m >= guard_var)
    if np.any(keep):
        Xc    = Xc[keep]
        ei    = ei[keep]
...
def main(argv=None):
    ...
    # Stopping heuristics
    ap.add_argument("--ei-stop", type=float, default=5e-2, help="If CEI's EI component < this and stable, stop.")
    ap.add_argument("--wrmsd-stop", type=float, default=0.1, help="If best m(Δ) ≤ this and EI small, stop.")
    # RNG
    ap.add_argument("--seed", type=int, default=23) #1337
