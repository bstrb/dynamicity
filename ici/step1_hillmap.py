
# step1_hillmap.py
from __future__ import annotations
import math
import random
from typing import List, Tuple, Optional
import numpy as np

class Trial:
    __slots__ = ("x_mm", "y_mm", "indexed", "wrmsd")
    def __init__(self, x_mm: float, y_mm: float, indexed: int, wrmsd: Optional[float]):
        self.x_mm = float(x_mm)
        self.y_mm = float(y_mm)
        self.indexed = int(indexed)
        self.wrmsd = None if wrmsd is None else float(wrmsd)

class Step1Params:
    __slots__ = (
        "radius_mm", "rng_seed", "n_candidates",
        "A0", "hill_amp_frac", "drop_amp_frac",
        "explore_floor", "min_spacing_mm",
        "first_attempt_center_mm",
        "allow_spacing_relax"
    )
    def __init__(
        self,
        radius_mm: float,
        rng_seed: int,
        n_candidates: int,
        A0: float,
        hill_amp_frac: float,
        drop_amp_frac: float,
        explore_floor: float,
        min_spacing_mm: float,
        first_attempt_center_mm: Tuple[float, float],
        allow_spacing_relax: bool = True,
    ):
        self.radius_mm = float(radius_mm)
        self.rng_seed = int(rng_seed)
        self.n_candidates = int(n_candidates)
        self.A0 = float(A0)
        self.hill_amp_frac = float(hill_amp_frac)
        self.drop_amp_frac = float(drop_amp_frac)
        self.explore_floor = float(explore_floor)
        self.min_spacing_mm = float(min_spacing_mm)
        self.first_attempt_center_mm = (float(first_attempt_center_mm[0]), float(first_attempt_center_mm[1]))
        self.allow_spacing_relax = bool(allow_spacing_relax)

def _gauss2d(x, y, cx, cy, sigma):
    dx = x - cx
    dy = y - cy
    return math.exp(-0.5 * (dx*dx + dy*dy) / (sigma*sigma))

def _sample_uniform_disk(n, R, rng: random.Random):
    theta = np.array([rng.uniform(0.0, 2*math.pi) for _ in range(n)], dtype=np.float64)
    r = np.array([R * math.sqrt(rng.random()) for _ in range(n)], dtype=np.float64)
    return np.stack([r * np.cos(theta), r * np.sin(theta)], axis=1)

def _filter_min_spacing(cands_xy, tried_xy, min_spacing):
    if tried_xy.size == 0:
        return np.arange(cands_xy.shape[0])
    diffs = cands_xy[:, None, :] - tried_xy[None, :, :]
    d2 = np.sum(diffs * diffs, axis=2)
    ok = np.all(d2 >= (min_spacing*min_spacing), axis=1)
    return np.where(ok)[0]

class Step1Result:
    __slots__ = ("done", "proposal_xy_mm", "reason")
    def __init__(self, done: bool, proposal_xy_mm: Optional[Tuple[float, float]], reason: str):
        self.done = bool(done)
        self.proposal_xy_mm = None if proposal_xy_mm is None else (float(proposal_xy_mm[0]), float(proposal_xy_mm[1]))
        self.reason = reason

def propose_step1(trials: List[Trial], params: Step1Params) -> Step1Result:
    rng = random.Random(params.rng_seed)
    R = params.radius_mm
    sigma = R / 2.0          # 2σ = R (fixed width)
    A0 = params.A0
    A_hill = params.hill_amp_frac * A0
    A_drop = - params.drop_amp_frac * A0

    tried_xy = np.array([[t.x_mm, t.y_mm] for t in trials], dtype=np.float64) if trials else np.empty((0,2), dtype=np.float64)
    successes = [(t.x_mm, t.y_mm, t.wrmsd) for t in trials if t.indexed == 1]
    failures  = [(t.x_mm, t.y_mm)        for t in trials if t.indexed == 0]

    cand_xy = _sample_uniform_disk(params.n_candidates, R, rng)

    keep = _filter_min_spacing(cand_xy, tried_xy, params.min_spacing_mm)
    if keep.size == 0 and params.allow_spacing_relax:
        keep = _filter_min_spacing(cand_xy, tried_xy, params.min_spacing_mm * 0.5)
    if keep.size == 0:
        return Step1Result(done=True, proposal_xy_mm=None, reason="step1_done_exhausted_no_candidates")

    cand_xy = cand_xy[keep, :]

    if trials:
        c0x, c0y = trials[0].x_mm, trials[0].y_mm
    else:
        c0x, c0y = params.first_attempt_center_mm

    w = np.zeros((cand_xy.shape[0],), dtype=np.float64)

    g0 = np.array([_gauss2d(x, y, c0x, c0y, sigma) for x, y in cand_xy], dtype=np.float64)
    w += A0 * g0

    if successes:
        for cx, cy, _ in successes:
            w += A_hill * np.array([_gauss2d(x, y, cx, cy, sigma) for x, y in cand_xy], dtype=np.float64)

    if failures:
        for cx, cy in failures:
            w += A_drop * np.array([_gauss2d(x, y, cx, cy, sigma) for x, y in cand_xy], dtype=np.float64)

    w = np.maximum(0.0, w) + params.explore_floor
    s = float(np.sum(w))
    if not np.isfinite(s) or s <= 0.0:
        return Step1Result(done=True, proposal_xy_mm=None, reason="step1_done_degenerate_weights")

    p = w / s
    np_rng = np.random.default_rng(params.rng_seed ^ 0xA53E12B4)
    idx = int(np_rng.choice(np.arange(cand_xy.shape[0]), p=p))
    x_mm, y_mm = float(cand_xy[idx, 0]), float(cand_xy[idx, 1])
    return Step1Result(done=False, proposal_xy_mm=(x_mm, y_mm), reason="step1_hillmap_sample")
