...
Now parses BOTH unit cell and geometry from the header blocks of a CrystFEL .stream:
    ----- Begin geometry file -----
...
def _extract_block(txt: str, begin_marker: str, end_marker: str) -> str:
    """
    Extract text between lines exactly matching the begin/end markers.
    Returns the inner content (lines between markers). Raises if not found.
    """
    # Use ^...$ with MULTILINE to be strict on full-line markers
    begin_re = re.compile(rf'^{re.escape(begin_marker)}\s*$', re.MULTILINE)
    end_re   = re.compile(rf'^{re.escape(end_marker)}\s*$', re.MULTILINE)

    mb = begin_re.search(txt)
    me = end_re.search(txt)
...
def parse_stream_headers(path_or_dash: str):
    """
    Read a CrystFEL .stream file (or stdin if '-') and extract the
    'geometry file' and 'unit cell' blocks from the header.
    """
    txt = _read_text(path_or_dash)

    # Prefer the first occurrence in the file; typical streams repeat headers,
    # but the first header is canonical for our purposes.
...
def main():
    ...
    # Parse inputs from stream header
    ...
    if args.csv:
        ...
        with open(csv_path, "w", newline="") as f:
            # Write custom header lines as comments
