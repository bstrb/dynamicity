...
now honoring crystal lattice centering (P, A, B, C, I, F, R).

Changes vs. original:
- Apply lattice-centering extinction rules when determining:
  (1) the first-ring (g_min, ring_mult), and
  (2) the crowding count N (|g| <= g_crowd).
- R-centering assumed in hexagonal axes: require h ≡ k ≡ l (mod 3).
...
def allowed_by_centering(h: int, k: int, l: int, centering: str) -> bool:
    """
    Return True if reflection hkl is *allowed* by lattice centering symbol.
    Implements standard extinction conditions for:
      P (primitive): all
      A: k + l even
      B: h + l even
      C: h + k even
      I: h + k + l even
      F: h,k,l all even OR all odd (unmixed parity)
      R: hexagonal axes assumption -> h ≡ k ≡ l (mod 3)
    """
    ...
    if c == "P":
        ...
        # Hexagonal axes setting for rhombohedral centering:
        # Require h ≡ k ≡ l (mod 3).
        return ((h - k) % 3 == 0) and ((k - l) % 3 == 0)
    else:
        # Unknown centering: be permissive (or change to False to be strict)
        return True
...
def _worker_init(Astar, nx, ny, clen_m, wavelength_A, res_px_per_m, g_enum, g_crowd, tol_g, centering):
    _G["Astar"] = Astar
    _G["nx"] = nx; _G["ny"] = ny
    _G["clen_m"] = clen_m
...
def _process_one_uvw(uvw):
    ...
    # Count N inside g_crowd (AFTER extinction)
    N = 0
    for h,k,l in hkls_allowed:
...
def main():
    ap = argparse.ArgumentParser(description="Predict problematic ZOLZ axes (Score=N, parallel, with centering extinctions).")
    ...
    print(f"Settings: UVW_MAX={args.uvw_max}, g_enum={g_enum:.4f}, g_crowd={g_crowd:.4f}, tol_g={tol_g} (fixed), Score=N | centering={centering}\n")
    ...
    with ctx.Pool(processes=os.cpu_count(),
                  ...
                  initargs=(Astar, nx, ny, geom["clen_m"], geom["wavelength_A"],
                            geom["res_px_per_m"], g_enum, g_crowd, tol_g, centering)) as pool:
    ...
    if not rows:
        fail("No candidate axes survived (extinctions and/or first ring off-panel).")
    ...
    # CSV (with header comments and triplet list)
    if args.csv:
        out_csv = os.path.splitext(args.stream if args.stream != "-" else "stdin.stream")[0] + "_problematic_axes.csv"
        with open(out_csv, "w", newline="") as f:
            # header comments
            f.write("# CrystFEL problematic zone axes prediction (with lattice-centering extinctions)\n")
            ...
            f.write(f"# Settings: UVW_MAX={args.uvw_max}, g_enum={g_enum:.4f}, g_crowd={g_crowd:.4f}, "
                    f"tol_g={tol_g} (fixed), Score=N | centering={centering}\n")
